...
async function carregarEscaloes() {
    try {
        ...
        snap.forEach(d => {
            ...
            btnDel.onclick = async () => { if(confirm(`Eliminar escalão ${nome}?`)) { await deleteDoc(doc(db, "escaloes", id)); carregarEscaloes(); } };
...
async function carregarTreinadores() {
    try {
        ...
        if (snap.empty) { listaTreinadoresCards.innerHTML = "<p style='color:#888; text-align:center;'>Nenhum treinador encontrado.</p>"; return; }
        ...
        snap.forEach(d => {
            ...
            card.innerHTML = `
                <div>
                    ...
                    <small style="color:#888; font-weight:500;">
                        ${t.escalao.toUpperCase()} • ${t.telefone} <br>
                        NIF: ${t.nif || '---'} | DOC: ${t.documento_id || '---'}
                    </small>
                </div>
            ...
            card.querySelector('.btn-edit-t').onclick = () => {
                ...
                document.getElementById('t-nacionalidade').value = t.nacionalidade || "";
                document.getElementById('t-nif').value = t.nif || "";
                document.getElementById('t-doc').value = t.documento_id || "";
            ...
            card.querySelector('.btn-del-t').onclick = async () => { if(confirm(`Remover ${t.nome}?`)) { await deleteDoc(doc(db, "treinadores", id)); carregarTreinadores(); } };
...
document.getElementById('btn-atualizar-t').onclick = async () => {
    ...
    const novosDados = {
        ...
        nacionalidade: document.getElementById('t-nacionalidade').value,
        nif: document.getElementById('t-nif').value,
        documento_id: document.getElementById('t-doc').value,
    ...
    resetarForm(); carregarTreinadores();
};
...
formTreinador.onsubmit = async (e) => {
    ...
    formTreinador.reset(); carregarTreinadores();
};
